...
- Implementer max_iter → `progress/_investigation_impl_N.md` (files read, command outcomes, last hypothesis — deterministic, no LLM); next attempt gets it injected as "PREVIOUS ATTEMPT'S INVESTIGATION — do not re-derive this". Complements `tool_call_errors`, which only re-feeds errors (an attempt with 175 clean tool calls left the retry nothing)
